	...
	db.on('value',function(chats){
		...
		chats.forEach(function(data){
			...
			if(chat.userId !== currentUserKey){
	    		...
	    		messageDisplay+=chat.msg;
	    		messageDisplay+='<span class="time float-right" title='+dateTime[0]+'>'+dateTime[1]+'</span></p></div>\
	        	       <div class="col-2 col-sm-1 col-md-1">\
	            	   <img src="'+firebase.auth().currentUser.photoURL+'" class="rounded-circle chat-pic"/></div>\
	               	   </div>';
	        } 
...
function SendMessage(){
	...
	firebase.database().ref('chatMessages').child(chatKey).push(chatMessage, function(error){
        if(error) alert(error);
        	...
        	//var pqr= document.getElementById("txtMessage").value;
	        //var message = '<div class="row justify-content-end">\
	        //              <div class="col-6 col-sm-7 col-md-7 ">\
	        //       	      <p class="sent float-right">';
	    	//message+=pqr;
	    	//message+='<span class="time float-right">1:28 PM</span></p></div>\
	        //	       <div class="col-2 col-sm-1 col-md-1">\
	        //    	   <img src="'+firebase.auth().currentUser.photoURL+'" class="rounded-circle chat-pic"/></div>\
	        //       	   </div>';
    ...
    db.on('value', function(users){
        ...
        users.forEach(function(data){
            ...
            if(user.email !== firebase.auth().currentUser.email){
            	lst +='<li class="list-group-item list-group-item-action" data-dismiss="modal" onclick="StartChat('+"'"+p+"','"+q+"','"+r+"'"+')">\
                       <div class="row">\
                       <div class="col-md-2">\
                       <img src="'+user.photoURL;
                lst+='"class="rounded-circle friend-pic"/>\
                       </div>\
                       <div class="col-md-10" style="cursor:pointer;">\
                       <div class="name">'+user.name+'</div>\
                       </div>\
